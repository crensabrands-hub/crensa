...
export default function VideoUpload({
    ...
    const validateMetadata = useCallback((): string[] => {
        ...
        // Free videos skip price validation
        if (metadata.isFree) {
            return errors
        }

        // For series videos (not free), coinPrice must be 0
        if (metadata.seriesId && metadata.coinPrice !== 0) {
            errors.push('Videos in a series must have a coin price of 0 (or be marked as free)')
        }

        // For standalone videos (not free), coinPrice must be 1-2000
        if (!metadata.seriesId && metadata.coinPrice < 1) {
            errors.push('Standalone videos must have a coin price of at least 1 (or be marked as free)')
        }
        ...
        return errors
    }, [metadata.seriesId, metadata.coinPrice, metadata.isFree])
 ...
 `}
                    ...
                    {uploadState.file && !uploadState.isUploading && (
                        ...
                        >
                            ...
                            {/* Category, Free Toggle, Coin Price, Series */}
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                        ...
                                        <button
                                            ...
                                            onClick={() => {
                                                setMetadata(prev => ({
                                                    ...prev,
                                                    isFree: !prev.isFree,
                                                    // Reset coinPrice when toggling free
                                                    coinPrice: !prev.isFree ? 0 : 20
                                                }))
                                                setValidationErrors([])
                                            }}
                                            className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 ${metadata.isFree ? 'bg-green-500' : 'bg-gray-200'
                                                }`}
                                        >
                                            <span
                                                className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${metadata.isFree ? 'translate-x-5' : 'translate-x-0'
                                                    }`}
                                            />
                                        </button>
                                        <span className={`text-sm font-medium ${metadata.isFree ? 'text-green-600' : 'text-gray-500'}`}>
                                            {metadata.isFree ? 'FREE' : 'Paid'}
                                        </span>
                                    </div>
                                    {metadata.isFree && (
                                        <p className="text-xs text-green-600 mt-1">
                                            Anyone can watch this video for free
                                        </p>
                                    )}
                                </div>
                                ...
                                <div>
                                    <CoinInput
                                        value={metadata.isFree ? 0 : (metadata.seriesId ? 0 : metadata.coinPrice)}
                                        onChange={(value) => {

                                            if (!metadata.seriesId && !metadata.isFree) {
                                                setMetadata(prev => ({
                                                    ...prev,
                                                    coinPrice: value,
                                        ...
                                        label="Coin Price"
                                        required={!metadata.isFree}
                                        showRupeeEquivalent
                                        placeholder="Enter coin price"
                                        disabled={!!metadata.seriesId || metadata.isFree}
                                    />
                                    {metadata.isFree && (
                                    ...
                                    {!metadata.isFree && metadata.seriesId && (
                                        ...
                                        onChange={(seriesId) => {
                                            ...
                                            setMetadata(prev => {
                                                const newMetadata = {
                                                    ...prev,
                                                    seriesId,
                                                    // Don't reset coinPrice/isFree when changing series
                                                    coinPrice: seriesId ? 0 : (prev.isFree ? 0 : (prev.coinPrice === 0 ? 20 : prev.coinPrice)),
                                                    creditCost: seriesId ? 0 : (prev.isFree ? 0 : (prev.creditCost === 0 ? 20 : prev.creditCost))
                                                }
