...
export default function TrendingCreatorsCarousel({
    ...
    const handleCreatorNavigate = (creator: Creator) => {
        if (onCreatorClick) {
            void onCreatorClick(creator);
        }
                    ...
                    <div className="relative overflow-hidden">
                        ...
                        >
                            {creators.map((creator, index) => (
                                ...
                                >
                                    <div
                                        className="bg-neutral-white rounded-2xl p-6 border border-neutral-light-gray hover:border-accent-pink hover:shadow-lg transition-all duration-300 group cursor-pointer"
                                        role="link"
                                        tabIndex={0}
                                        onClick={() => handleCreatorNavigate(creator)}
                                        onKeyDown={(event) => {
                                            if (event.key === 'Enter' || event.key === ' ') {
                                                event.preventDefault();
                                                handleCreatorNavigate(creator);
                                            }
                                        }}
                                    >
                                        <div className="flex items-center gap-4 mb-4">
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            <button
                                                ...
                                                className={`px-4 py-2 rounded-lg font-medium transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed ${creator.isFollowing
                    ...
                    <div className="flex justify-center gap-2 mt-6">
                        {Array.from({ length: maxIndex + 1 }).map((_, index) => (
                            <button
                                ...
                                className={`w-2 h-2 rounded-full transition-all duration-300 ${index === currentIndex ? 'bg-accent-pink' : 'bg-neutral-light-gray'
                                    }`}
                            />
                        ))}
                    </div>
