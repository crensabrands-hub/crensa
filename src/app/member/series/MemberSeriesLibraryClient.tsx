...
export default function MemberSeriesLibraryClient({
  ...
  const getButtonText = (progress: SeriesProgress): string => {
    if (progress.videosWatched === 0) return 'Start Watching';
    if (progress.videosWatched >= progress.totalVideos) return 'Watch Again';
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="max-w-7xl mx-auto px-4 py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl lg:text-4xl font-bold text-primary-navy mb-2">My Series</h1>
          <p className="text-neutral-dark-gray">Your purchased series collection</p>
        </div>
        ...
        {purchasedSeries.length === 0 ? (
          ...
          >
            <div className="max-w-md mx-auto">
              ...
              <h2 className="text-2xl font-bold text-primary-navy mb-3">No Series Yet</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {purchasedSeries.map((item, index) => {
              ...
              const videosLeft = progress.totalVideos - progress.videosWatched;
                ...
                >
                  ...
                  <Link href={`/series/${series.id}`} className="block relative">
                    <div className="aspect-video bg-neutral-light-gray relative overflow-hidden group">
                      ...
                      {/* Hover overlay — desktop pointer devices only */}
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/50 flex flex-col items-center justify-center gap-3 transition-colors duration-200">
                        <motion.div
                        ...
                        {/* Watch progress tooltip on hover */}
                        <div className="opacity-0 group-hover:opacity-100 transition-opacity bg-black/80 backdrop-blur-sm text-white rounded-lg px-4 py-2 text-center pointer-events-none">
                          <div className="text-primary-neon-yellow font-semibold text-sm">
                            {progress.videosWatched} / {progress.totalVideos} watched
                          </div>
                          <div className="text-white/75 text-xs mt-0.5">
                            {videosLeft} video{videosLeft !== 1 ? 's' : ''} left to watch
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  {/* Mobile/tablet progress strip — visible only on touch/no-hover devices */}
                  <div className="flex items-center justify-between bg-primary-navy/5 border-b border-neutral-light-gray px-4 py-2 md:hidden [@media(hover:hover)]:hidden">
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-white rounded-lg p-4 text-center">
                ...
                <div className="text-sm text-neutral-dark-gray">Series Owned</div>
              </div>
              <div className="bg-white rounded-lg p-4 text-center">
                <div className="text-3xl font-bold text-accent-pink mb-1">
                  {purchasedSeries.reduce((sum, item) => sum + item.series.videoCount, 0)}
                </div>
                <div className="text-sm text-neutral-dark-gray">Total Videos</div>
              </div>
              <div className="bg-white rounded-lg p-4 text-center">
                ...
                <div className="text-sm text-neutral-dark-gray">Average Progress</div>
              </div>
            </div>
          </motion.div>
        )}
