...
export async function POST(request: NextRequest) {
 ...
 const emailSubject = `${getPriorityEmoji(data.priority)} Creator Help Request: ${data.subject}`;
 ...
 <head>
 <meta charset="utf-8">
 <title>Creator Help Request</title>
 <style>
 ...
 <div class="container">
 <div class="header">
 <h1>🎬 Creator Help Request</h1>
 <p>New support request from Crensa Creator Platform</p>
 </div>
 ...
 <div class="info-grid">
 <div class="info-item">
 <div class="label">Creator Details</div>
 ...
 <div class="footer">
 <p>This email was sent from the Crensa Creator Help System.</p>
 <p>Please respond directly to this email to assist the creator.</p>
 ...
 const emailText = `
Creator Help Request - ${data.priority.toUpperCase()} PRIORITY
...
Response Time Target: ${data.priority === 'urgent' ? 'Within 2 hours' : 
 ...
 const mailOptions = {
 from: `"Crensa Creator Platform" <${process.env.ADMIN_EMAIL_USER || 'noreply@crensa.com'}>`,
 to: adminEmail,
 ...
 'X-Creator-ID': data.userProfile.id,
 ...
 console.log('=== CREATOR HELP REQUEST (DEV MODE) ===');
 console.log('To:', adminEmail);
 console.log('Subject:', emailSubject);
 console.log('From Creator:', data.userProfile.username, `(${data.userProfile.email})`);
 ...
 console.log(`Help request submitted by ${data.userProfile.username} (${data.userProfile.email}): ${data.subject}`);

 return NextResponse.json({
 success: true,
 message: 'Help request sent successfully',
 ticketId: `HELP-${Date.now()}-${data.userProfile.id.slice(-4)}` // Simple ticket ID
 });
