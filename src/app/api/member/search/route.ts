...
import { eq, and, or, ilike, gte, lte, desc, asc, count, inArray } from "drizzle-orm";
...
const VALID_SORT_OPTIONS = [
 ...
 "title",
 "relevance",
 "following", // Member-specific: prioritize followed creators
];
...
export async function GET(request: NextRequest) {
 ...
 if (user.role !== "member") {
 return NextResponse.json(
 { success: false, error: "Only members can access this search" },
 { status: 403 }
 );
 }
 ...
 if (sortBy === "following" || followingOnly) {
 const followedCreators = await db
 .select({ creatorId: creatorFollows.creatorId })
 .from(creatorFollows)
 .where(eq(creatorFollows.followerId, user.id));
 
 followedCreatorIds = followedCreators.map(f => f.creatorId);
 }

 const purchasedVideos = await db
 .select({ videoId: transactions.videoId })
 .from(transactions)
 .where(
 and(
 ...
 purchasedVideos.map(p => p.videoId).filter(Boolean) as string[]
 );
 ...
 if (followingOnly && followedCreatorIds.length > 0) {
 videoWhereConditions = and(
 videoWhereConditions,
 inArray(videos.creatorId, followedCreatorIds)
 );
 }
 ...
 const hasAccess = purchasedVideoIds.has(result.id);
 const isFollowing = followedCreatorIds.includes(result.creatorId);
 ...
 if (followingOnly && followedCreatorIds.length > 0) {
 seriesWhereConditions = and(
 seriesWhereConditions,
 inArray(series.creatorId, followedCreatorIds)
 );
 }
 ...
 memberData: {
 followedCreatorsCount: followedCreatorIds.length,
 purchasedVideosCount: purchasedVideoIds.size,
 },
 });
 } catch (error) {
